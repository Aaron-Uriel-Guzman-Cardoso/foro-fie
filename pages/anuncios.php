...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['comment_submit'])) {
    ...
    if (!empty($content)) {
        try {
            ...
            // Insertar comentario
            $stmt_insert = $conn->prepare("INSERT INTO `comment` (`post`, `account`, `publication`, `content`) VALUES (:post, :account, NOW(), :content)");
            ...
            if ($parent_id !== null) {
                $stmt_reply = $conn->prepare("INSERT INTO `reply` (`comment`, `parent`) VALUES (:comment, :parent)");
                $stmt_reply->bindParam(':comment', $new_comment_id, PDO::PARAM_INT);
                $stmt_reply->bindParam(':parent', $parent_id, PDO::PARAM_INT);
                $stmt_reply->execute();
            }

            header("Location: anuncios.php#post-$post_id");
            exit();
...
try {
    ...
    // Consultar publicaciones de la categoría "Anuncios" incluyendo el ID
    $query_publicaciones = "
        SELECT 
            p.id,
            p.title, 
...
function get_all_comments_with_replies($conn, $post_id) {
    ...
    $stmt_comments = $conn->prepare("SELECT * FROM `comment` WHERE `post` = :post_id ORDER BY `publication` ASC");
    ...
    $stmt_replies = $conn->prepare("SELECT * FROM `reply` WHERE `parent` IN ($placeholders)");
    ...
    foreach ($comments_map as $comment) {
        ...
        $stmt_check_parent = $conn->prepare("SELECT * FROM `reply` WHERE `comment` = :comment_id");
    ...
    foreach ($comments as $comment) {
        ...
        <li class="comentario">
            ...
            <div class="reply-form" id="reply-form-<?= $comment['id'] ?>">
                <form action="anuncios.php#post-<?= $post_id ?>" method="POST" class="form-comentario">
                ...
                <?php foreach ($publicaciones as $publicacion): ?>
                    <div class="publicacion" id="post-<?= $publicacion['id'] ?>">
                        ...
                        <div class="comentarios">
                            ...
                            <form action="anuncios.php#post-<?= $publicacion['id'] ?>" method="POST" class="form-comentario">
